...
export default function Home() {
  ...
  const { getTodayContact, deferContact, getInteractionsForContact, data } = useApp();
  const [deferMessage] = useState(() => DEFER_MESSAGES[Math.floor(Math.random() * DEFER_MESSAGES.length)]);
  const [showDeferMsg, setShowDeferMsg] = useState(false);
  const [currentContactId, setCurrentContactId] = useState<string | null>(null);

  const contact = currentContactId
    ? data.contacts.find((c) => c.id === currentContactId) ?? getTodayContact()
    : getTodayContact();
    ...
    return (
      <View className="flex-1 bg-linen" style={{ paddingTop: insets.top }}>
        <View className="flex-row justify-end px-6 pt-4 pb-2">
          <TouchableOpacity onPress={() => router.push('/settings')} activeOpacity={0.6} className="p-2">
        ...
        <View className="flex-1 items-center justify-center px-10 pb-28">
          ...
          >
            <Text
              ...
              style={{ fontFamily: 'Fraunces_400Regular', fontSize: 22, lineHeight: 36, opacity: 0.5 }}
            >
          ...
          <View className="py-8">
            <View className="items-center mb-16">
              ...
              <Text
                ...
                style={{ fontFamily: 'PlusJakartaSans_400Regular', fontSize: 14, opacity: 0.4, lineHeight: 24 }}
              >
                {contact.tier}
              </Text>
              {lastNote ? (
              ...
              {sinceLabel ? (
                <Text
                  className="text-ink"
                  style={{ fontFamily: 'PlusJakartaSans_400Regular', fontSize: 14, opacity: 0.35, lineHeight: 24 }}
                >
                  {sinceLabel}
                </Text>
              ) : null}
      ...
      {showDeferMsg && (
        ...
        >
          <Text
            ...
            style={{ fontFamily: 'Fraunces_400Regular', fontSize: 18, opacity: 0.7, lineHeight: 32 }}
          >
            {deferMessage}
          </Text>
        </MotiView>
